...
namespace NinjaBattle.UI.Editor
{
    public static class Vertical3x3Authoring
    {
        ...
        private static RectTransform ButtonNode(string name, Transform parent, Color color) { Image image = Panel(name, parent, color); image.raycastTarget = true; image.gameObject.AddComponent<Button>().transition = Selectable.Transition.ColorTint; UIButtonJuice.Attach(image.gameObject, 0.94f); return image.rectTransform; }
